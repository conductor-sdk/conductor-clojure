...
(defn runner-executor-for-workers
  ...
  [workers options]
  (-> (task-client options)
      (task-runner-configurer (map utils/clj-worker->Worker workers))
      (doto (.init))))
(comment
  (def worker
{
...
(def instance (runner-executor-for-workers (list worker) {:url  "http://localhost:8080/api/" :app-key "f082aa94-ba42-4f95-9d9f-c808b3fd7485" :app-secret "JkSxYI8D8YIpcuNnSeBbPS9ug1rVPqii3Xia2nGRE1ICcepW"} ))
(.shutdown instance)
)
